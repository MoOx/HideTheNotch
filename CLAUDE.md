...
The icon has one source too, `tools/brand.cjs`, and what sits in `assets/` is
its output: every platform's version of it, the iOS `.icon` layers and the
Android ones alike, comes from one drawing there. Never edit an asset by hand
or add a second recipe for a platform. Run `npm run brand` and commit what it writes;
