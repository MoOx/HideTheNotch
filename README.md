...
| `ios-testflight.yml` | macos | manual, or `[testflight]` | signed build shipped to TestFlight, about 8 min |
| `android-play.yml` | ubuntu | manual, or `[play]` | signed AAB on the Play internal track |
...
| App Privacy answers, published | App Store Connect > App Information > App Privacy, by an Admin. Crash data, for app functionality, not linked to you, not used for tracking | Apple will not accept a submission without them, and this is not the privacy policy URL |
...
The tag starts nothing. It used to: both build workflows fired on `refs/tags/v*`,
which made an accidental push, a moved tag, or a tag written a day early into a
release. They are started from the Actions tab now, deliberately, and the tag is
written afterwards to say what went out. `npm run release` without `--push`
writes it and leaves it here, if you would rather not push it at all.

What `release` is really for is the four things it refuses: a dirty tree, a
version already tagged, a version not bumped, and a release note nobody
rewrote. That last one is the reason it exists.
