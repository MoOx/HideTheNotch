...
`npm run listing` writes both trees:

```sh
npm run deck        # the screenshots, from the captures
npm run listing     # the two metadata trees, from listing.json
```

The trees are not committed, `marketing/listing.json` is. Both lanes regenerate
them before uploading, so there is no second copy of six languages to keep in
step. The generator also enforces every store limit (30 characters for a name,
80 for Play's short description, 100 for Apple's keywords, and so on) and
refuses to write anything if one is over, since finding out at upload time costs
a review queue.

`fastlane ios metadata` pushes the App Store listing and the deck without
building anything, which is what a correction to the copy actually needs.
...
which is not the same as being on sale:
...
withdrawn quietly, so it is a separate word: `HTN_SUBMIT=1 npm run promote:ios`.
Even then the version waits for a tap before going on sale.

`promote:android` moves the artefact between tracks, so what reaches production
is the binary that was tested. `HTN_ROLLOUT=0.1 npm run promote:android` gives it
to a tenth of users first, and Play holds the rest until the rollout is finished
by hand.
